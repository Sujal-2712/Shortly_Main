...
class UrlController {
  ...
  try {
    ...
    // Convert uploaded QR to base64, or generate one
    if (qrCodeBuffer) {
    ...
    // If customUrl is not provided, generate a unique short ID
  ...
  async redirectUrl(req, res) {
    try {
      ...
      const urlData = await URL.findOne({ short_url: shortUrl })
        .select('_id original_url');
      console.log(urlData);
      if (!urlData) {
        return res.status(404).json({
          success: false,
